...
const HelpDeskTable = () => {
  ...
  const tableHeaders = ["Configuration Name", "District(s)", "Protocols"];
  ...
  const helpDeskUsersData = useSelector(
    (state) => state?.superAdmin?.helpDeskUsers?.data?.response
  );
  ...
  const handleSuperAdminAddition = () => {
    dispatch(
      setManageUser({
        userType: "helpDesk",
        formTitle: `Add Help Desk User`,
        previousPath: location.pathname,
      })
    );

    navigate("/role/SuperAdmin/AddUser", { state: { role: "helpDesk" } });
  };

  useEffect(() => {
    dispatch(getAllHelpDeskUsers({ token: token }));
  }, []);

  useEffect(() => {
    setData(helpDeskUsersData);
  }, [helpDeskUsersData]);

  console.log("data of helpDesk users===", data);

  const RedirectToHelpDeskUser = (userInfo) => {
    console.log(
      "Help Desk User Information from Manage Users Component",
      userInfo
    );
    dispatch(getUserData({ id: userInfo?.uuid, token }));
    dispatch(
      setManageUser({
    ...
    navigate(`/role/${userRole}/edit/helpDesk/${userInfo?.uuid}`);
  };

  console.log("HelpDesk User Data", helpDeskUsersData);
      ...
      <TableContainer marginTop="10" className="SuperAdminTables">
        <Table variant="stripped" color="">
          <Thead>
            <Tr>
              <Th>
                <Text textStyle="h4">Configuration Name</Text>
              </Th>
              <Th>
                <Text textStyle="h4">District(s)</Text>
              </Th>
              <Th>
                <Text textStyle="h4">Protocol</Text>
              </Th>
            </Tr>
          </Thead>
          <Tbody>
              ...
              data?.map((item, key) => (
                <Tr>
                  <Td color="#1890ff">
                    ...
                    <Button onClick={() => RedirectToHelpDeskUser(item)}>
                      {item.user_name}
                    </Button>
                  </Td>
                  <Td>{}</Td>
