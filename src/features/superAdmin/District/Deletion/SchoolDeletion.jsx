...
const SchoolDeletion = (props) => {
  console.log("props in school deletion", props);
  const finalRef = React.useRef(null);
  const cardData = "Remove District";
  ...
  const deleteSchoolResponse = useSelector(
    (state) => state?.superAdmin?.removeSchool
  );
  ...
  const handleRemove = () => {
    ...
    console.log("handle Remove function====", updated_by);
    dispatch(
      getRemovedSchool({
        uuid: uuid,
        token: token,
              ...
              <Box>
                ...
                <Center>
                  <Text alignContent="space-around">
                    All the data associated with 1 District Administrator will
                    be Removed.
                  </Text>
                </Center>
                <Center>
